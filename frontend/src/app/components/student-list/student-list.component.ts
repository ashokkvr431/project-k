...
export class StudentListComponent implements OnInit {
  ...
  loadMergedStudents() {
    this.studentService.getMergedStudents().subscribe(mergedData => {
      ...
      this.filteredMergedStudents = [...this.merge_list];

      this.uniqueCourses = [...new Set(this.merge_list.map(stu => stu.course))];
  ...
  filterMergedByCourse() {
    if (this.selectedCourse) {
      this.filteredMergedStudents = this.merge_list.filter(stu => stu.course === this.selectedCourse)
    } else {
      this.filteredMergedStudents = [...this.merge_list]
    }
  }
